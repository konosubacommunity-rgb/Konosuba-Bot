...
interface UserData {
  ...
  rank?: number;
  joinedDate?: string;
  inventory?: { pokemon?: unknown[]; [key: string]: unknown };
  stats?: {
    fishCaught?: number;
    itemsDug?: number;
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (!token || !currentUser) { setLocation("/auth"); }
  ...
  const loadData = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      if (actData.status === "fulfilled") setActivities(actData.value?.activities || []);
      if (lbData.status === "fulfilled") setLeaderboard(lbData.value?.users || lbData.value?.leaderboard || []);
    } catch (e: unknown) {
  ...
  const xp = user?.xp ?? 0;
  const xpNeeded = user?.xpNeeded ?? 100;
        ...
        <div className="nav-actions">
          {user && (
            <div className="db-user-chip">
              ...
              <div>
                <div className="db-user-name">{user.username || currentUser?.username || "Adventurer"}</div>
          ...
          <>
            <div className="welcome-banner">
              ...
              <div className="welcome-text" style={{ flex: 1 }}>
                <h2>Welcome back, {user?.username || currentUser?.username || "Adventurer"}! ⚔️</h2>
                ...
                <div style={{ display: "flex", gap: ".5rem", flexWrap: "wrap" }}>
                  ...
                  <span className="stat-badge">🔥 Streak <strong>{user?.dailyStreak || 0}d</strong></span>
            ...
            <div className="cards-grid">
              ...
              <StatCard icon={<Star size={16} />} label="Net Worth" value={formatMoney(user?.totalBalance || 0)} sub="Total balance" color="#ffb74d" />
            </div>
                ...
                {activities.length === 0 ? (
                  ...
                  <div className="activity-item" key={a._id || i}>
                    ...
                    <div className="act-text">
                      <div className="act-desc">{a.description || a.type || "Activity"}</div>
                      <div className="act-time">{a.createdAt ? formatTime(a.createdAt) : ""}</div>
                    </div>
                    {a.amount != null && (
                ...
                <div className="panel">
                  ...
                  {leaderboard.length === 0 ? (
                    ...
                    <div className="lb-item" key={u.phone || i}>
                      ...
                      <div className="lb-val">{formatMoney(u.totalBalance || 0)}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="panel">
                  ...
                  <div className="stats-list">
                    {[
                      ...
                      { icon: <Sparkles size={13} />, val: user?.stats?.pokemonCaught || 0, lbl: "Pokémon Caught" },
                      { icon: <TrendingUp size={13} />, val: user?.stats?.timesGambled || 0, lbl: "Gambles Played" },
                      { icon: <Users size={13} />, val: user?.inventory?.pokemon ? (user.inventory.pokemon as unknown[]).length : 0, lbl: "Pokémon Owned" },
                    ].map((s, i) => (
