...
router.post('/auth/login', async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { phone: cleanPhone, jid: user.jid, _id: user._id },
      JWT_SECRET,
      { expiresIn: '7d' }
    );
...
router.post('/auth/register', async (req, res) => {
  try {
    ...
    if (user) {
      ...
      user = new User({ jid, name: name || cleanPhone, password, registered: true });
      await user.save();
    }

    const token = jwt.sign({ phone: cleanPhone, jid: user.jid, _id: user._id }, JWT_SECRET, { expiresIn: '7d' });
    res.json({ token, user: serializeUser(user) });
  } catch (err) {
...
// FIX: normalize activity fields — DB stores {desc, timestamp}, frontend expects {description, createdAt}
...
// FIX: add totalBalance field alongside netWorth so both field names work on the frontend
router.get('/leaderboard', async (req, res) => {
  try {
    const users = await User.find({}, {
      jid: 1, lid: 1, name: 1, wallet: 1, bank: 1, level: 1, xp: 1,
    }).sort({ wallet: -1 }).limit(50).lean();

    const result = users.map((u, i) => {
      const netWorth = (u.wallet || 0) + (u.bank || 0);
      const phone = u.jid ? u.jid.split('@')[0] : (u.lid ? u.lid.split('@')[0] : '');
      return {
        rank:         i + 1,
        name:         u.name || phone,
        phone,
        ...
        bank:         u.bank || 0,
        netWorth,
        totalBalance: netWorth,   // alias so frontend works with either field name
      };
    });
...
// GET /api/website/admin/users?page=1&limit=20&search=
  ...
  try {
    ...
    if (search) {
      ...
      query.$or = [
        ...
        ...(phone ? [{ jid: { $regex: `^${phone}` } }, { lid: { $regex: `^${phone}` } }] : []),
      ];
    }
...
// GET /api/website/admin/user/:phone
...
// PUT /api/website/admin/edit-user
...
// POST /api/website/admin/ban-user
...
// POST /api/website/admin/unban-user
...
// POST /api/website/admin/reset-cooldowns
...
// DELETE /api/website/admin/delete-user
...
// POST /api/website/admin/wipe-economy
...
// POST /api/website/admin/wipe-xp
...
// POST /api/website/admin/wipe-inventory
...
// GET /api/website/admin/export-users
...
function serializeUser(user) {
  ...
  const phone = obj.jid ? obj.jid.split('@')[0] : (obj.lid ? obj.lid.split('@')[0] : '');
  return {
    _id:      obj._id,
    phone,
    name:     obj.name || phone,
