...
async function handleRpg(sock, message, command, args, sender, isGroup, groupJid) {
  ...
  if (command === 'dungeon') {
    ...
    if (win) {
      ...
      user.rpg.hp = Math.max(1, user.rpg.hp - randomInt(20, 50));
      await user.save();
      await sock.sendMessage(dest, {
  ...
  if (command === 'craft') {
    ...
    const craftable = [
      { name: 'health potion', materials: ['herb', 'water'], result: 'Health Potion', cost: 50 },
      { name: 'iron sword', materials: ['iron', 'iron'], result: 'Iron Sword', cost: 100 },
    ];
